...
public class ComDetailController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		delete.setOnAction(e -> {
			...
			try {
				cnt = service.deleteBoard(DataBoard.board.getBoardNum());
		...
		update.setOnAction(e -> {
			...
			try {
				parent = FXMLLoader.load(getClass().getResource("detailBoard.fxml"));
			} catch (IOException e2) {
				e2.printStackTrace();
			}

			BoardVO vo = new BoardVO();
			vo.setBoardNum(DataBoard.board.getBoardNum()); // vo에 저장된 num을 가져오는것
