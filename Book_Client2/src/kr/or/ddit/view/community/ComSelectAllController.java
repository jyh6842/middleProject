...
public class ComSelectAllController implements Initializable {
	...
	public void setdate(int cnt) {	// 초기데이터값 세팅
		try {
			List<BoardVO> list = (List<BoardVO>) service.detailBoard(cnt);
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		table.setOnMouseClicked(e->{
			...
			Parent root;
			try {
				root = FXMLLoader.load(getClass().getResource("detailBoard.fxml"));
...
//		table.setItems(b);
		...
		write.setOnAction(e -> {
			Parent root;
			try {
				root = FXMLLoader.load(getClass().getResource("writeCom.fxml"));
				Scene scene = new Scene(root);
				Stage stage = (Stage) ((Node) e.getSource()).getScene().getWindow();
				stage.setScene(scene);
				stage.show();
		...
		public void searchList() {
			...
			try {
				b.clear();
				vo = service.selectBoard(searchTitle);
